...
(function (root, factory) {
   ...
   * Marks restore complete when no pending history remains, or when
   * options.forceCompleteRestore is true (grace timeout).
   *
   ...
   *   3) Assign remaining live tabs with no matched history.
   *   4) Keep unmatched stored records with non-empty URLs as
   *      pendingRestore placeholders so progressive Chrome/Firefox
   *      session restore can claim them on a later sync.
   */
  function mergeLiveTabsWithHistory(liveTabs, storedList, options) {
    ...
    // Re-key when the original storage key is already used by a live tab.
    if (mode === "restore") {
      for (const sk of Object.keys(stored)) {
        if (usedStoredKeys.has(sk)) continue;
        const rec = stored[sk];
