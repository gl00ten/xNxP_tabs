...
(function (root, factory) {
   ...
   *   Only match by exact tab id. Never move history between two open tabs
   *   that share a URL.
   *
   * mode "restore" (startup only):
   *   1) Match by exact tab id
   *   2) Match remaining live tabs to remaining stored records by URL
   *      (prefer oldest firstOpenedTs for each URL)
   */
  function mergeLiveTabsWithHistory(liveTabs, storedList, options) {
    ...
    for (let i = 0; i < liveTabs.length; i++) {
      ...
      if (stored[key] && !usedStoredKeys.has(key)) {
