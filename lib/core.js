...
(function (root, factory) {
   ...
   */
  function getLastOpenedTsForTab(tab, tabInfoList, nowMs) {
  ...
  /**
   * Exact-URL duplicates. Keep most recently accessed (+ all active).
   */
  function analyzeDuplicates(tabs, tabInfoList, nowMs) {
    const byUrl = new Map();
    ...
    for (const group of byUrl.values()) {
      ...
      let best = group[0];
      let bestTs = getLastOpenedTsForTab(best, tabInfoList, nowMs);
      for (let i = 1; i < group.length; i++) {
        const tab = group[i];
        const ts = getLastOpenedTsForTab(tab, tabInfoList, nowMs);
        if (ts > bestTs) {
          best = tab;
          bestTs = ts;
        }
  ...
  /**
   * Chrome unload path: filter unloadable ids (never active), then discard one-by-one.
   */
  async function unloadListedForChrome(liveTabs, listedIds, discardFn, keepTabId) {
    const ids = selectUnloadListedIds(liveTabs, listedIds, keepTabId);
    return discardTabIds(ids, discardFn, { oneByOne: true });
  }
  ...
  return {
    ...
    unloadListedForChrome: unloadListedForChrome,
